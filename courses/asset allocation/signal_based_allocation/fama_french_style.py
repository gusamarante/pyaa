...
# ============================
# TODO turn this into a function/class
# def max_exposure(signal, c, covariance, target_vol=0.1):
#
#     constraints = ({'type': 'eq',
#                     'fun': lambda w: np.sqrt(w.T @ covariance @ w) - target_vol})
#
#     bounds = Bounds(- c * np.ones(signal.shape[0]), c * np.ones(signal.shape[0]))
#
#
#     # Run optimization
#     res = minimize(lambda w: w.T * s,  # TODO parei aqui
#                    w0,
#                    method='SLSQP',
...
# start_date = carry.index[0]
# notional_start = 100
# weights = max_exposure(carry.loc[start_date], 1, covar.loc[start_date])
# holdings = notional_start * weights / trackers.loc[start_date]
...
#
#     if d >= next_roll_date:
#         weights = max_exposure(carry.loc[dm1], 1, covar.loc[dm1])
